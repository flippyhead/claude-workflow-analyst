...
function processCompareResponse(data, plugin) {
    // For monorepo plugins, filter files to only those under the plugin path
    if (plugin.repoPath) {
        const relevantFiles = data.files.filter((f) => f.filename.startsWith(plugin.repoPath));
...
async function fetchCompareCommits(plugin, sinceDays, headers) {
...
async function fetchDateScopedCommits(plugin, sinceDays, headers) {
...
async function fetchDedicatedRepoUpdates(plugin, sinceDays, headers) {
...
async function fetchMonorepoUpdates(plugin, sinceDays, headers) {
...
export async function fetchPluginUpdates(plugins, sinceDays) {
    const releases = [];
    const errors = [];
    ...
    for (const plugin of plugins) {
        ...
        try {
            ...
            if (result.rateLimited) {
                rateLimited = true;
                errors.push(`GitHub rate limit hit while checking plugin ${plugin.name}. ` +
            ...
            errors.push(`Failed to fetch updates for plugin ${plugin.name}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    const pluginsWithUpdates = new Set(releases.map((r) => r.repo)).size;
    return { releases, pluginsWithUpdates, rateLimited, errors };
}
//# sourceMappingURL=plugin-releases.js.map