...
export function deduplicate(insights, state) {
    const now = Date.now();
    ...
    return insights.filter((insight) => {
        ...
        if (entry.dismissCount >= MAX_DISMISS_COUNT)
            return false;
        // Suppress if seen recently (within staleness window)
        const lastSeen = new Date(entry.lastSeen).getTime();
        if (now - lastSeen < TWO_WEEKS_MS)
            return false;
        return true;
    });
}
export function updateState(state, publishedInsights) {
