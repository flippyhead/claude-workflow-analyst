...
export class DirectAutomationAnalyzer implements Analyzer {
  ...
  private detectConfirmations(input: AnalyzerInput): Insight[] {
    // Count single-word confirmation messages
    const toolConfirmations = new Map<string, number>();
  ...
  private async detectRepeatedPrompts(input: AnalyzerInput): Promise<Insight[]> {
    ...
    if (frequentPrompts.length === 0) return [];
...
Only include prompts worth automating. Skip trivial ones. Return [] if none qualify.`;
    ...
    try {
      const result = await input.llm.analyzeStructured<Insight[]>(prompt, { type: "array" });
  ...
  private async detectRepeatedSequences(input: AnalyzerInput): Promise<Insight[]> {
    ...
    if (frequentSequences.length === 0) return [];
