...
export class AIBrainOutput implements OutputTarget {
  name = "ai-brain";
  ...
  async publish(insights: Insight[], metadata: ReportMetadata): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[ai-brain] Non-fatal: failed to publish report (HTTP ${response.status})`
        );
      }
    } catch (err) {
      // Non-fatal — AI Brain may not be running
      console.warn(
        `[ai-brain] Non-fatal: could not reach endpoint at ${this.endpoint}`,
